...
This project supports two tenant isolation modes using a simple toggle. The toggle is **only for routing** and **does
not implement schema mode yet**.
...
- **schema**: `TenantHibernateFilter` is disabled. This prevents column-filter logic from running when you implement
  schema routing later.
